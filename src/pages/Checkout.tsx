...
const Checkout: React.FC = () => {
  ...
  const handlePlaceOrder = () => {
    const order = {
      ...
      items: cart.items,
      total: cart.items.reduce((sum, item) => sum + Math.floor(item.price * item.quantity), 0),
      user,
    };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center py-8">
      ...
      <div className="bg-white rounded-lg shadow p-8 w-full max-w-lg">
        ...
        <div className="font-bold text-xl mb-6">
          Total: ${cart.items.reduce((sum, item) => sum + Math.floor(item.price * item.quantity), 0).toFixed(2)}
        </div>
        <Button
          id="place-order-btn"
